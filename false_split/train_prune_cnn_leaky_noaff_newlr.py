...
from prepare_data_3d_my_noaff import *
...
def weighted_mse(y_true, y_pred):
    ...
    neg_weight = maybe_print(1.0 / (2 * neg_fracs), "negative weight", do_print=True) # was 1.05
...
if __name__=='__main__':
    ...
    doFineTune = args.doFineTune #False
    ...
    gen_data = GenerateData(train_trial,train_imagedir, train_segname, train_skfile, patchSize, patchZ)

    #pdb.set_trace()
    if doFineTune and os.path.exists(train_trial):
        ...
        print "== BLOCK 1 =="
        input = Input(shape=(3, patchZ, patchSize, patchSize))

        n_initial_kernels = 28
        layer=1
    ...
    translate_endpt_flag = np.random.choice([0,1],2000000,p=[0.75,0.25]) 

    for epoch in xrange(st_epoch,st_epoch+200100):
        print
        print "iteration: "+str(epoch)
        
        #generate data from different blocks

        data = gen_data.get_3d_sample2(nsamples_batch=batch_sz,translate_endpt=translate_endpt_flag[epoch])

        #pdb.set_trace()
        if epoch<5:
            fidw=h5py.File('train_samples/input_sample_'+str(epoch)+'.h5','w')
        ...
        current_lr = (learning_rate_init /(1. + (epoch - st_epoch)*decay_rate)) 
